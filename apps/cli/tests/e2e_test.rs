...
fn execute_search(index_name: &str, query: &str) -> Result<Value, Box<dyn std::error::Error>> {
    ...
    if !search_output.status.success() {
        eprintln!("beetle search for '{}' failed:", query);
        eprintln!("stdout: {}", String::from_utf8_lossy(&search_output.stdout));
        eprintln!("stderr: {}", String::from_utf8_lossy(&search_output.stderr));
        return Err(format!("beetle search command for '{}' failed", query).into());
    }

    let search_stdout = String::from_utf8_lossy(&search_output.stdout);
    let json_result: Value = serde_json::from_str(&search_stdout)?;
...
fn validate_search_results<'a>(
    ...
    assert_eq!(
        results.len(),
        expected_count,
        "Search results for '{}' should contain exactly {} result(s)",
        query,
        expected_count
    );
...
fn validate_result_by_file(
    ...
    let result = results
        ...
        .unwrap_or_else(|| {
            panic!(
                "Should find {} in search results for '{}'",
                file_name, query
            )
        });
    ...
    assert!(
        ...
        "Search results for '{}' should contain '{}' in {} snippet",
        query,
        expected_snippet,
        file_name
    );
